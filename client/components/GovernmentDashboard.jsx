...
const GovernmentDashboard = ({ user }) => {
    ...
    const handleUpdateStatus = async (issueId, newStatus) => {
        try {
            await axios.put(`/issues/${issueId}`, { status: newStatus });
            toast.success("Issue status updated successfully!");
            getIssues(); // Refresh the list
        } catch (err) {
        ...
        <div className="p-4 space-y-4">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {issues.map(issue => (
                        <Card key={issue._id} className="p-4 cursor-pointer hover:bg-gray-50" onClick={() => openDialog(issue)}>
                            <div className="flex flex-col space-y-2">
                                <CardTitle>{issue.title}</CardTitle>
                                <CardDescription>Category: {issue.category}</CardDescription>
                                <CardDescription>Reporter: {issue.reporter?.username}</CardDescription>
                                <Badge variant="default">{issue.status}</Badge>
                                <Select onValueChange={(newStatus) => handleUpdateStatus(issue._id, newStatus)}>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogContent className="max-w-xl p-0">
                <DialogHeader className="p-6 pb-2">
                  ...
                  Category: <span className="font-normal ml-1">{selectedIssue?.category}</span>
                </p>
                </DialogHeader>
